...
#轴承数据
data1= np.loadtxt("z1.csv",delimiter=",")

train_x_disorder =data1[0:1100,0:64]
test_x_disorder =data1[1100:,0:64]
train_y_disorder =data1[0:1100,-1]
train_y_disorder=np.reshape(train_y_disorder,(1100,1))
# train_y_disorder=train_y_disorder*10
test_y_disorder =data1[1100:,-1]
test_y_disorder =np.reshape(test_y_disorder,(150,1))
# test_y_disorder =test_y_disorder *10
#cnn
# 变厚矩阵
...
xs = tf.placeholder(tf.float32, [None, 64])  # 原始数据的维度：16
ys = tf.placeholder(tf.float32, [None, 1])  # 输出数据为维度：1

keep_prob = tf.placeholder(tf.float32)  # dropout的比例

x_image = tf.reshape(xs, [-1, 8, 8, 1])  # 原始数据16变成二维图片4*4
## conv1 layer ##第一卷积层
...
h_poo22 = max_pool_2x2(h_conv2)     # output size 2x2x32 长宽缩小一倍
#第二层池化
# h_poo22=tf.nn.max_pool(h_conv2, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')

## fc1 layer ##  full connection 全连接层
W_fc1 = weight_variable([2 * 2 * 64, 512])  # 4x4 ，高度为64的三维图片，然后把它拉成512长的一维数组
b_fc1 = bias_variable([512])

h_pool2_flat = tf.reshape(h_poo22, [-1, 2 * 2 * 64])  # 把4*4，高度为64的三维图片拉成一维数组 降维处理
